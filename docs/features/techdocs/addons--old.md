...
| [`<DocumentHistory />`](https://backstage.io/api/stable/variables/_backstage_plugin-techdocs-module-addons-contrib.index.DocumentHistory.html)           | `@backstage/plugin-techdocs-module-addons-contrib` | Optional document history for TechDocs pages: commit history, tags, file compare, blame, and browsing a selected revision while navigating within the same documentation site. Requires `techdocs.history.enabled: true`, a page `edit_uri`, and a configured GitHub or GitLab integration.                                                                                                                                       |
